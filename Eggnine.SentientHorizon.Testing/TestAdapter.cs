...
using Eggnine.SentientHorizon.Testing.SkipAttributes;
...
public class TestAdapter
{
    ...
    public TestAdapter()
    {
        ...
        foreach (JToken child in token.Children())
        {
            if (child is JProperty property)
            {
                ...
                {
                    case "browserName":
                        TestContext.BrowserName = property.Value.ToString();
                        break;
                    case "browserMajorVersion":
                        TestContext.BrowserMajorVersion = property.Value.ToString();
                        break;
                    case "operatingSystemNameAndMajorVersion":
                        var splitOsAndVersion = property.Value.ToString().Split("-");
                        TestContext.OperatingSystemName = splitOsAndVersion[0];
                        TestContext.OperatingSystemMajorVersion = splitOsAndVersion[1];
                        break;
                    case "isHeadless":
                        bool.TryParse(property.Value.ToString(), out TestContext.IsHeadless);
                        break;
                    case "gitHubUsername":
                        TestContext.GitHubUsername = property.Value.ToString();
                        break;
                    case "gitHubPassword":
                        TestContext.GitHubPassword = property.Value.ToString();
                        break;
                    case "databaseConnectionString":
                        TestContext.DatabaseConnectionString = property.Value.ToString();
                        break;
                    default:
                        break;
                }
        ...
        TestContext.LogMessage($"Browser: {TestContext.BrowserName} {TestContext.BrowserMajorVersion}");
        TestContext.LogMessage($"Operating System: {TestContext.OperatingSystemName} {TestContext.OperatingSystemMajorVersion}");
    }
    ...
    public void ExecuteTest(Action<TestContext> test)
    {
        var ctx = TestContext;
        ...
        TestContext.LogMessage($"Begining pre-execution for test case #{ctx.TestCaseId} {ctx.ClassName}.{ctx.TestName}");
        IEnumerable<SkipIf> skipIfs = testMethod.GetCustomAttributes().Where(a => a is SkipIf).Cast<SkipIf>();
        TestContext.LogMessage($"Checking if test case should be skipped");
        ...
        while (true)
        {
            try
            {
                TestContext.LogMessage("Starting WebDriver");
                WebDriver webDriver;
                ...
                TestContext.LogMessage("WebDriver started");
                try
                {
                    TestContext.LogMessage("Executing test action");
                    test(ctx);

                    TestContext.LogMessage("Begining post execution");
    ...
    private void RecordTestResult(TestContext ctx)
    {
    ...
    /// <returns></returns>
    private WebDriver StartWebDriverWithRetries(TestContext ctx, Func<WebDriver> webDriverFunc)
    {
        int i = 0;
        while (true)
        {
    ...
    /// </summary>
    /// <param name="test"></param>
    public static void Execute(Action<TestContext> test) => new TestAdapter().ExecuteTest(test);
}
